...
import { search, view, create, update } from '../../services/deliveryGoods';
...
const initialState = {
  query: {},
  deliveryGoods: [{id:1, name: '123', deliverNum: 123}],
  current: {},
  pagination: {
    current: 1,
  },
...
export default {
  ...
  subscriptions: {
    listSubscriptions({ dispatch, history }) {
      history.listen((location, state) => {
        if (pathToRegexp('/order/orders/:id/deliverys/:id/deliveryGoods').test(location.pathname)) {
          const match = pathToRegexp('/order/orders/:id/deliverys/:id/deliveryGoods').exec(location.pathname);
          const id = match[1];
          console.log(match)
          dispatch({ type: 'clear' })
          dispatch({
    ...
    itemSubscriptions({ dispatch, history }) {
      history.listen((location, state) => {
        if (pathToRegexp('/order/orders/:id/deliverys/:id/deliveryGoods/:action+').test(location.pathname)) {
          dispatch({ type: 'clear' })
        }
      })
    }, 
    ...
    editSubscriptions({ dispatch, history }) {
      history.listen((location, state) => {
        if (pathToRegexp('/order/orders/:id/deliverys/:id/deliveryGoods/edit/:id').test(location.pathname)) {
          const match = pathToRegexp('/order/orders/:id/deliverys/:id/deliveryGoods/:id').exec(location.pathname);
          const id = match[1];
          dispatch({
            type: 'view',
            payload: id,
  ...
  effects: {
    setQuery: [function*({ payload: query }, { put, select }) {
      ...
      yield message.error(`加载本次送货的商品列表失败:${err.status} ${err.message}`, 3);
      return false;
    }, { type: 'takeLatest' }],
    *view({ payload: id }, { put, select }) {
      ...
      const err = yield parseError(error);
      yield message.error(`加载本次送货的商品失败:${err.status} ${err.message}`, 3);
      return false;
    },
    *add({ payload }, { put, call, select }) {
      ...
      if (!error) {
    	yield message.success('创建本次送货的商品成功', 2);
        yield put(routerRedux.goBack());
      } else {
    	const err = yield parseError(error);
        yield message.error(`创建本次送货的商品失败:${err.status} ${err.message}`, 3);
      }
    ...
    *edit({ payload }, { put, select }) {
      ...
      if (!error) {
	    yield message.success('更新本次送货的商品成功', 2);
        yield put(routerRedux.goBack());
      } else {
        const err = yield parseError(error);
        yield message.error(`更新本次送货的商品失败:${err.status} ${err.message}`, 3);
      }
  ...
  reducers: {
    ...
    setDeliveryNum(state, { payload }) {
      const deliveryGoods = state.deliveryGoods
      const target = deliveryGoods.map(item => {
        if (item.id == payload.id) {
          return { ...item, deliveryNum: payload.value }
        }
      })
      return { ...state, deliveryGoods: target }
    },

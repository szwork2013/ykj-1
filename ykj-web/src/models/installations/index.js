  ...
  subscriptions: {
    listSubscriptions({ dispatch, history }) {
      history.listen((location, state) => {
        if (pathToRegexp('/order/orders/:id/installations').test(location.pathname)) {
          const match = pathToRegexp('/order/orders/:id/installations').exec(location.pathname);
          const id = match[1];
          dispatch({ type: 'clear' })
          dispatch({
            type: 'setQuery',
    ...
    editSubscriptions({ dispatch, history }) {
      history.listen((location, state) => {
        if (pathToRegexp('/order/installations/edit/:id').test(location.pathname)) {
          const match = pathToRegexp('/order/installations/edit/:id').exec(location.pathname);
          const id = match[1];
          dispatch({
            type: 'view',
            payload: id,
    ...
    setQuery: [function*({ payload: query }, { put, select }) {
      ...
      if (!error) {
        yield put({
          type: 'setMeasures',
          payload: {
            installations: data._embedded && data._embedded.orderSers || [],
            pagination: {

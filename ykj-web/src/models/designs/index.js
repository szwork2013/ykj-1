...
import { root, search, view, create, update, remove, removeAll, statement, cancelStatement } from '../../services/design';
  ...
  subscriptions: {
    listSubscriptions({ dispatch, history }) {
      history.listen((location, state) => {
        if (pathToRegexp('/order/orders/:id/designs').test(location.pathname)) {
          const match = pathToRegexp('/order/orders/:id/designs').exec(location.pathname);
          const id = match[1];
          dispatch({ type: 'clear' })
          dispatch({
            type: 'setQuery',
    ...
    editSubscriptions({ dispatch, history }) {
      history.listen((location, state) => {
        if (pathToRegexp('/order/designs/edit/:id').test(location.pathname)) {
          const match = pathToRegexp('/order/designs/edit/:id').exec(location.pathname);
          const id = match[1];
          dispatch({
            type: 'view',
            payload: id,
    ...
    setQuery: [function*({ payload: query }, { put, select }) {
      ...
      if (!error) {
        yield put({
          type: 'setMeasures',
          payload: {
            designs: data._embedded && data._embedded.orderSers || [],
            pagination: {
